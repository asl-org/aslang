...
type
  ...
  ResolvedExpression* = ref object of RootObj
    ...
    of REK_LITERAL_INIT: literal: ResolvedLiteral
...
proc new_resolved_expression*(literal: ResolvedLiteral): ResolvedExpression =
  ResolvedExpression(kind: REK_LITERAL_INIT, literal: literal)
...
proc return_type*(expression: ResolvedExpression): Token =
  case expression.kind:
  of REK_VARIABLE: expression.variable.typ
...
proc c*(expression: ResolvedExpression): string =
  let native_code =
    case expression.kind:
    of REK_VARIABLE: $(expression.variable.name)
    ...
    of REK_LITERAL_INIT: $(expression.literal.value)
