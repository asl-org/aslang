...
type
  ...
  AnalyzedCasePattern* = ref object of RootObj
    ...
    of RCPK_LITERAL:
      ...
      literal: Literal
    of RCPK_UNION:
      union_pattern: AnalyzedUnionPattern
...
proc new_analyzed_case_pattern(union_pattern: AnalyzedUnionPattern,
    location: Location): AnalyzedCasePattern =
  AnalyzedCasePattern(kind: RCPK_UNION, union_pattern: union_pattern,
      location: location)
...
proc literal*(pattern: AnalyzedCasePattern): Literal =
  ...
  pattern.literal
proc union_pattern*(pattern: AnalyzedCasePattern): AnalyzedUnionPattern =
  do_assert pattern.kind == RCPK_UNION, "expected union pattern"
  pattern.union_pattern

proc hash*(pattern: AnalyzedCasePattern): Hash =
  case pattern.kind:
  of RCPK_LITERAL: pattern.literal.asl.hash
  of RCPK_UNION: pattern.union_pattern.hash
...
proc generic_impls*(pattern: AnalyzedCasePattern): Table[ResolvedModule, seq[
  ...
  of RCPK_UNION: impl_set = impl_set.merge(pattern.union_pattern.generic_impls)
  return impl_set

proc asl*(case_pattern: AnalyzedCasePattern): string =
  case case_pattern.kind:
  of RCPK_LITERAL: case_pattern.literal.asl
  of RCPK_UNION: case_pattern.union_pattern.asl
...
proc analyze*(file_def: AnalyzedFileDefinition, scope: FunctionScope,
  ...
  of CPK_STRUCT:
    ...
    let analyzed_union_pattern = ? analyze(file_def, scope, operand, struct)
    ok(new_analyzed_case_pattern(analyzed_union_pattern, pattern.location))
