import strformat, sequtils, algorithm, sets
...
proc generate_struct_defs*(struct: AnalyzedStruct, prefix: string,
    generics: uint64 = 0,
    manual_extern_names: HashSet[string] = initHashSet[string]()): seq[CDecl] =
  ...
  # Generate init unless a manually-defined ExternFunction provides it
  # (e.g., Array_init is defined in runtime.c with custom allocation logic)
  if fmt"{prefix}_init" notin manual_extern_names:
...
proc generate_branch_defs*(branch: AnalyzedUnionBranch, prefix: string,
    id: uint64, generics: uint64 = 0,
    manual_extern_names: HashSet[string] = initHashSet[string]()): seq[
        CDecl] =
  ...
  # Generate branch init unless manually defined
  if fmt"{sub_prefix}_init" notin manual_extern_names:
